...
void main() {
  ...
  blocTest<TvDetailBloc, TvDetailState>(
    ...
    expect: () => [
      TvDetailLoading(),
      TvDetailHasData(tvDetail: testTvDetail, recommendations: testTvList)
    ],
    verify: (_) {
  ...
  blocTest<TvDetailBloc, TvDetailState>(
    'Should emit Loading, HasData when data is gotten unsuccesful',
    build: () {
      when(mockGetTvDetail.execute(tId))
  ...
  blocTest<TvDetailBloc, TvDetailState>(
    'Should emit Loading, HasData when tv detail is gotten unsuccesful',
    build: () {
      when(mockGetTvDetail.execute(tId))
          .thenAnswer((_) async => Left(ServerFailure('Server Failure')));
      when(mockGetTvRecommendations.execute(tId))
  ...
  blocTest<TvDetailBloc, TvDetailState>(
    'Should emit Loading, HasData when tv recommendation is gotten unsuccesful',
    build: () {
      when(mockGetTvDetail.execute(tId))
          .thenAnswer((_) async => Right(testTvDetail));
      when(mockGetTvRecommendations.execute(tId))
